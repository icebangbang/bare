...
function widthbox(){
    ...
    var win_h = $(window,document).height(); //屏幕当前宽度
    $(".xn_c_contentwarp").width(win_w); //主盒子宽度等于屏幕宽度
    $(".xn_c_content").width(win_w-222);

    // 高度实时调节
    $(".xn_c_content").height(win_h-92);
...
$(function(){
    ...
    // 2017-09-01 侧面导航切换路由时恢复定位
    setTimeout(function () {
        $('.xn_c_content_left').scrollTop(localStorage.getItem('leftScrollTopVal'))
        $('.xn_c_li_head_a').each(function (index, item) {
            if (item.href === location.href) {
                $(this).addClass('active')
            }
        })
        $('.xn_c_content_left').css('opacity', 1)
        $('.xn_c_content_left').scroll(function () {
            var leftScrollTopVal = $('.xn_c_content_left').scrollTop()
            localStorage.setItem('leftScrollTopVal', leftScrollTopVal)
        })
    })
});
